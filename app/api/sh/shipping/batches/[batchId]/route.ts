...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  if (body?.status === 'COMPLETED') {
    ...
    try {
      ...
      // 재고 차감 실패 사유(미사용 상품·비활성 위치 등)를 그대로 노출 — 500 로 뭉개면 원인 파악 불가
      if (err instanceof MovementError) return errorResponse(err.message, err.status)
      console.error('[PATCH /api/sh/shipping/batches/[batchId]] 완료 실패', err)
      return errorResponse('처리 완료에 실패했습니다', 500)
    }
  }
