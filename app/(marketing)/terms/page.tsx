...
const sections = [
  ...
  {
    title: '제2조 (정의)',
    body: `① "서비스"란 회사가 제공하는 광고 분석, 재고·배송 운영, 재무 관리, 채용, 세일즈 콘텐츠, 블로그 운영 등 업무별 모듈("deck")과 이를 이용하기 위한 워크스페이스 일체를 의미합니다.
② "이용자"란 이 약관에 따라 회사와 이용계약을 체결하고 서비스를 이용하는 개인 또는 법인을 의미합니다.
③ "워크스페이스"란 이용자가 소속되어 하나 이상의 deck을 이용하는 단위 공간을 의미합니다.`,
  },
  {
...
② 회사는 관련 법령을 위배하지 않는 범위에서 이 약관을 개정할 수 있으며, 개정 시 적용일자 및 개정사유를 명시하여 최소 7일 전(이용자에게 불리한 변경의 경우 30일 전)부터 서비스 내 공지 또는 이메일로 안내합니다.`,
  },
  {
    ...
    body: `① 회사는 이용자가 필요한 deck을 선택하여 이용할 수 있도록 서비스를 제공합니다.
② 현재 모든 deck은 베타 기간 동안 무료로 제공되며, 정식 유료 전환 시점과 조건은 사전에 공지합니다.
...
③ 이용계약 종료 시 이용자 데이터의 보관·파기 정책은 개인정보처리방침에 따릅니다.`,
  },
  {
    ...
    body: `① 회사는 정식 유료 전환 이후 deck별 구독료를 부과할 수 있으며, 요금 및 결제 방식은 서비스 내 요금제 페이지에서 안내합니다.
② 결제는 토스페이먼츠 등 회사가 지정하는 전자결제 서비스를 통해 이루어집니다.
③ 이용자는 구독 중인 deck을 언제든지 해지할 수 있으며, 해지 시점과 환불 정책은 관련 법령 및 회사가 정한 기준에 따릅니다.`,
  },
  {
    title: '제7조 (이용자의 의무)',
