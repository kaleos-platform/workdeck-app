...
> **2026-09-24 정정.** 아래 §1의 귀속률(직배송 미매칭 61%, 로켓 36%, 광고비 35%)은
> **측정 방법이 틀렸거나 구현 결함 때문에 나온 값**이다. 지금은 해당하지 않는다.
>
> - **직배송 61% 미매칭** — `margin-query` 가 `DelOrderItemFulfillment` 를 조회하지 않아
>   listingId 없이 fulfillment 만 달린 라인(90일 1,635건)을 전부 미매칭으로 버린 결과였다.
>   매출·수량을 `loadProductSales` 단일 소스로 바꾼 뒤 **직배송 귀속률 99.9%**.
> - **로켓 36% / 광고비 35%** — 2026-08 `InvLocationProductMap` 대량 백필 이전 수치.
>   현재 로켓 매출 100%, 광고비 99%.
> - 같은 수정에서 **로켓 수수료가 0원으로 계산되던 버그**(`name.includes('로켓그로스')` 가
>   수수료 0% 채널을 먼저 잡음)와 **배송비 개당 부과**(주문당이어야 함)도 고쳤다.
>
> 최신 실측과 배경은 메모리 `project_sales_analytics_product_ranking` 및 가이드
> `guides/판매분석 상품 미매칭 정리 가이드.md` 참조.
