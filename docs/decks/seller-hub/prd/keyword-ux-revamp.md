...
- **링크는 한 행에 `productId`·`listingId` 를 함께 담는다.** 위 설계는 두 행(리스팅 링크 + 상품 링크)을 적었지만, 한 행으로도 `?productId=`·`?listingId=` 두 필터가 모두 만족되고 쓰기량은 절반이다. 대가: 리스팅이 삭제되면 cascade 로 상품 귀속도 함께 사라진다 — 같은 상품의 다른 리스팅이 저장될 때 다시 쌓이므로 영구 손실은 아니다.
