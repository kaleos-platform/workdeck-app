...
### 2026-09-23 Proxy 인증 후속 최신 판정

- [x] PR #939~#942 초기화 세부 계측 및 쿠팡 화면 Proxy 중복 인증 조회 개선 운영 배포.
- [x] 관련105 tests·lint·build·typecheck·리뷰, DEV 실제 refresh/폐기 차단 확인과 계정 삭제.
- [x] 고정 배포 URL/운영 도메인 각각5회 측정, API200·KPI 일치 및 비로그인 로그인 이동/API401 확인.
- [x] 상세 목표: 운영 진입 최대2.729초, 재진입 최대0.595초.
- [ ] 홈 재진입1초 안정화: 운영5회 중1.416/1.425초 초과. Proxy는2.8/4.6ms, RSC 응답 약0.9초여서 서버 초기화·레이아웃과 요청별 연결 추적이 남음.
- [ ] 직접 진입3초 안정화: 운영5회 최대2.840초, 고정 URL 첫 데이터 DOM2.727초. 기존 load 포함 측정은3.221초여서 초과 표본을 유지함.

측정 commit `be8a87c9`와 원시값·브라우저 timeout 등 한계는 성능 검증 가이드에 기록했다.
전체 목표 완료로 처리하지 않는다. 이 최신 판정이 앞선 날짜별 상태보다 우선한다.

### 2026-09-24 최신 진단

- [x] x-vercel-id로 브라우저 RSC와 SSR 로그를 연결. 느린 RSC819/867ms에서 멤버십586/602ms 및 Prisma 생성190/194ms 확인.
- [x] prefetch 차단5쌍 및 Prisma7.4.1/7.10.0 새 프로세스5쌍 비교. 일관된 개선 근거가 없어 앱 정책·의존성 유지.
- [x] E2E 데이터 표시/load 대기 분리, 요청 식별자 attachment 추가. 실제 측정 callback CDP 실행5회·데이터 일치·수집 실패0, lint/typecheck/리뷰 확인.
- [ ] 초기화가 포함된 첫 화면3초 안정화: 고정 배포 데이터 DOM3.433초 표본 남음.
- [ ] 재진입1초 안정화: warm 검사는 모두 충족했으나 상세1.936초 초기화 표본도 새로 확인. 앞선 상세 완료 표시는 이 최신 미완료 판정으로 대체.
- [ ] 같은 코드로 Standard2GB/1vCPU와 Performance4GB/2vCPU 비교 후보. 프로젝트 전체 향후 배포·비용에 영향을 주므로 설정 변경 확인 전에는 적용하지 않음.

실험 원시값·요청 연결 증거·측정 한계·복원 조건은 성능 검증 가이드에 기록했다.
