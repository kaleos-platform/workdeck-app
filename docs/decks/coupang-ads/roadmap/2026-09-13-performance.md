...
- [ ] 홈 서버에서 기존 workspace 범위의 KPI·목록 조회를 병렬로 시작해 초기 결과를 전달한다.
- [ ] 클라이언트는 초기 결과를 즉시 표시하고 기존 API 재검증을 계속한다. 기간 변경 중에는 이전 기간 숫자를 표시하지 않으며, 오래된 응답이 새 기간을 덮지 않게 한다.
- [ ] 초기값 표시, 서버 props 갱신, 날짜 변경, 재검증 결과 반영 회귀 테스트를 먼저 작성한다.
- [ ] 홈 E2E는 특정 API 완료 대신 실제 KPI·카드 표시를 기준으로 측정하도록 수정한다. API 정확성 검사는 별도로 유지한다.
- [ ] 검증·리뷰·배포 후 동일 조건으로 재측정한다. 인증/권한 검사를 생략하거나 사용자 정보를 전역 cache에 저장하지 않는다.
