...
| 서비스                         | 현재 소유                | 목표 소유              | 이유                                          |
| ------------------------------ | ------------------------ | ---------------------- | --------------------------------------------- |
| **GitHub** (`kaleos-platform`) | `devops@kaleos.co.kr`    | 유지                   | 법인 코드 자산 — 이전 완료 ✅                 |
| **Supabase**                   | `devops@kaleos.co.kr`    | 유지                   | DB = 법인 핵심 인프라 자산                    |
| **Stripe**                     | (미생성)                 | `devops@kaleos.co.kr`  | 결제·정산 = 법인 명의 필수                    |
| **Vercel**                     | `eddy.sangwon@gmail.com` | 유지 (팀 합류 시 이전) | 1인 단계 무료 유지                            |
| **Google Cloud (OAuth)**       | `eddy.sangwon@gmail.com` | `devops@kaleos.co.kr`  | 인증 인프라 → 법인 소유 권장, **지금 불필요** |
| **Sentry**                     | (개인)                   | `devops@kaleos.co.kr`  | 법인 단위 통합 관리 (여러 제품 프로젝트)      |
| **도메인** `workdeck.work`     | `devops@workdeck.work`   | 유지                   | 제품 브랜드 도메인                            |
| **도메인** `kaleos.co.kr`      | `devops@kaleos.co.kr`    | 유지                   | 법인 도메인                                   |

#### 실제 로그인 흐름

```
개발 작업   → eddy.sangwon@gmail.com  (GitHub commit/push, Vercel 확인)
인프라 관리 → devops@kaleos.co.kr     (Supabase, Stripe, GitHub 조직 설정, Sentry 조직 소유)
서비스 운영 → devops@workdeck.work    (DNS, 고객 커뮤니케이션, Sentry Admin)
```
...
| Sentry   | Kaleos 조직 생성 + 프로젝트 생성  | `devops@kaleos.co.kr`    |

> **Vercel 업그레이드 시점**: 팀원이 Vercel 대시보드에 직접 접근할 필요가 생길 때 Pro Team($20/월)으로 전환한다.

**이전 순서 원칙**: GitHub ✅ → Supabase → Vercel → Sentry → Google Cloud (나중에)
(Vercel이 GitHub 레포에 의존하므로 GitHub 이전 선행 필수. Google Cloud는 Redirect URI 추가만으로 당장 운영 가능)
...
> **Google Cloud OAuth 앱 이전 계획**: 현재 개인 계정(`eddy.sangwon@gmail.com`) 소유로 두어도 동작에 문제 없다.
> 조직 계정 이전은 **0-A-7 섹션** 참조. 지금 당장 필요 없으며, Redirect URI 추가만으로 충분하다.
...
GitHub(`kaleos-platform`) / Supabase(`Kaleos`)와 일관되게 **Kaleos 조직**으로 생성한다.
여러 제품(Workdeck, 미래 제품)의 프로젝트를 한 조직에서 통합 관리한다.

> **소유 계정**: `devops@kaleos.co.kr` (법인 인프라 통합 관리)

**목표 구조**:

```
Account: devops@kaleos.co.kr (Owner)
└── Organization: Kaleos
    ├── Members:
    │   devops@kaleos.co.kr   → Owner
    │   devops@workdeck.work  → Admin  (제품 운영, 에러 대응)
    │   eddy.sangwon@gmail.com → Admin (개발 작업)
    └── Projects:
        ├── workdeck-app        (현재 — Next.js)
        └── [미래 제품]-app    (확장 시 추가)
```

**방법: Kaleos 조직 + 프로젝트 생성** (이력 불필요, 새 DSN 발급):

```
sentry.io → devops@kaleos.co.kr 로그인
→ New Organization → "Kaleos"
→ Members → Invite:
    devops@workdeck.work   (Admin)
    eddy.sangwon@gmail.com (Admin)
→ Projects → Create Project
→ Platform: Next.js 선택
→ Project name: workdeck-app
...
Auth Token   → SENTRY_AUTH_TOKEN
              (devops@kaleos.co.kr → User Settings → Auth Tokens → Create New Token)
Project name → SENTRY_PROJECT  (workdeck-app)
```

**기존 개인 Sentry 프로젝트 처리**:

```
새 Kaleos 조직 프로젝트 정상 동작 확인 후
→ 기존 개인 계정 workdeck 프로젝트 삭제 또는 비활성화
```

---

#### 0-A-4-B. Google Cloud OAuth 앱 조직 이전 (나중에)

> **이전 시점**: 팀 합류, 법인 명의 OAuth 앱 필요 시, 또는 개인 계정 정리 시점에 진행.
> 지금 당장은 **기존 개인 Google Cloud 앱에 Redirect URI만 추가**하면 충분하다.

**이전이 필요한 상황:**

- 팀원이 Google Cloud Console 접근 필요
- 개인 계정(eddy.sangwon@gmail.com) 정리 예정
- 법인 명의 OAuth 앱으로 브랜드 일관성 확보 원할 때

**이전 절차:**

```
1. Google Cloud Console → devops@kaleos.co.kr 로그인
2. New Project → "Workdeck" 생성
3. APIs & Services → Credentials → OAuth 2.0 Client ID 생성
   - Application type: Web application
   - Name: Workdeck OAuth
   - Authorized redirect URIs:
       https://[workdeck-prod-ref].supabase.co/auth/v1/callback
       https://[workdeck-dev-ref].supabase.co/auth/v1/callback
       https://app.workdeck.work/auth/callback
       http://localhost:3000/auth/callback
4. Client ID / Client Secret 복사
```

```
5. Supabase workdeck-prod → Authentication → Providers → Google
   → Client ID / Secret 교체 → Save
6. Supabase workdeck-dev → 동일하게 교체
7. 기존 개인 계정 OAuth 앱 비활성화 또는 삭제
```

> **주의**: Supabase에 등록된 redirect URI 형식은 `https://{ref}.supabase.co/auth/v1/callback`이며,
> Google Cloud Console의 Authorized redirect URIs에 이 URL이 반드시 포함되어야 한다.
...
- [x] GitHub `kaleos-platform/workdeck-app` 레포 접근 가능
- [x] 로컬 `git remote -v` → `github.com/kaleos-platform/workdeck-app`
- [x] `workdeck-dev` Supabase: `npx prisma migrate deploy` 성공
- [x] `workdeck-prod` Supabase: 프로젝트 생성 완료, 환경변수 수집 완료
- [x] Vercel → kaleos-platform/workdeck-app 레포 재연결 확인
- [x] Vercel Production/Preview 환경변수 각각 prod/dev Supabase로 설정
- [x] Google OAuth redirect URI 각 Supabase 프로젝트에 등록 완료 (kaleos 계정 Google Cloud 앱, URI 추가)
- [ ] (나중에) Google Cloud OAuth 앱 devops@kaleos.co.kr 조직 계정으로 이전 → 0-A-4-B 참조
- [x] Sentry: Kaleos 조직 생성 (devops@kaleos.co.kr Owner), workdeck-app 프로젝트 생성
- [x] Sentry: devops@workdeck.work Admin 초대 완료
- [x] Sentry: 새 DSN / Org Slug / Auth Token 환경변수 반영 (.env.local + Vercel)
- [x] `npm run build` 통과
- [x] 로컬 `npm run dev` → 로그인 페이지 로드, 미인증 /dashboard → /login 리다이렉트 정상
...
- [x] `https://app.workdeck.work` 접속 → 앱 정상 로드
- [x] Google OAuth 로그인 → `/auth/callback` 정상 처리
- [x] Vercel Production/Preview 환경변수 분리 확인
- [x] Preview 배포 URL에서도 로그인 정상 동작 (workdeck-dev 연결 확인)
...
### Phase 2. Core OS 레이어 전환 (3~5일)

**목표**: `resolveWorkspace()` → `resolveDeckContext()` 점진 전환 + RBAC

#### 2-1. `resolveDeckContext()` (`src/lib/api-helpers.ts`)
...
#### 2-2. RBAC 가드 (`src/lib/api-helpers.ts`)
...
#### 2-3. Same-space 검증 (`src/lib/api-helpers.ts`)
...
### Phase 3. 앱 셸 UI 구현 (3~5일)

**목표**: My Deck 홈, Deck 진입 라우트 구현 (빌링 UI는 Phase 5에서 추가)
...
> `/billing` 라우트는 Phase 5 (Stripe 연동) 완료 후 추가

#### 3-2. Sidebar 업데이트 (`src/components/layout/sidebar.tsx`)
...
### Phase 4. RLS (Row Level Security) 설정 (1~2일)
...
### Phase 5. Stripe 결제 연동 (5~7일) — PoC 검증 후 진행

> **진행 조건**: Phase 3 (앱 셸 UI) 완료 후, 여러 카드(coupang-ads, osmu 등)의 PoC 사용성 검증이 완료된 시점에 시작합니다.
> MVP 단계에서는 결제 없이 운영하며, 실제 사용자 피드백을 통해 과금 모델을 확정합니다.

**목표**: 구독 기반 결제 시스템 + Feature Gating + 빌링 페이지 UI
...
#### 5-5. Feature Gating (`src/lib/entitlement.ts`)
...
#### 5-6. 빌링 페이지 (`app/billing/page.tsx`)
...
#### 5-7. Sidebar 빌링 링크 추가 (`src/components/layout/sidebar.tsx`)
...
Phase 2 (3~5일)
  ...
  사용량 미터링 (MeterEvent)
        ↓
Phase 3 (3~5일)
  /my-deck, /d/[deckKey] 라우트
  Sidebar 업데이트 (빌링 링크 제외)
        ↓
Phase 4 (1~2일)
  RLS Supabase 정책 설정
        ↓
      [PoC 검증 — 여러 카드 사용성 확인 후 진행]
        ↓
Phase 5 (5~7일)
  Stripe 상품/가격 설정 (Dashboard)
  Checkout / Webhook / Portal API 구현
  Feature Gating (entitlement.ts)
  빌링 페이지 UI + Sidebar 링크 추가
        ↓
Phase 6 (별도)
  workdeck.work 마케팅 사이트 (별도 레포)

총 예상: 10~17일 (Phase 4까지 MVP) + Phase 5 (5~7일, PoC 검증 후)
...
MVP Phase 5에서는 **Starter/Pro 2개 플랜만** 구현 (Team은 이후 추가).
...
- [x] GitHub `kaleos-platform/workdeck-app` 레포 접근 가능 (Transfer 완료)
- [x] 로컬 `git remote -v` → `github.com/kaleos-platform/workdeck-app`
- [x] `workdeck-dev` Supabase (Kaleos 조직): `npx prisma migrate deploy` 성공
- [x] `workdeck-prod` Supabase (Kaleos 조직): 프로젝트 생성 + 환경변수 수집 완료
- [x] Vercel: kaleos-platform/workdeck-app 레포 재연결 확인 (개인 계정 유지)
- [x] Vercel: Production → workdeck-prod, Preview → workdeck-dev 환경변수 적용
- [x] Google OAuth redirect URI: 각 Supabase 프로젝트에 등록
- [x] Sentry: Kaleos 조직(devops@kaleos.co.kr) workdeck-app 프로젝트 생성 + 새 DSN 환경변수 반영
- [x] `npm run build` 통과
- [x] 로컬 `npm run dev` → 로그인 페이지 로드, 미인증 /dashboard → /login 리다이렉트 정상
...
- [x] `npx prisma migrate deploy` 성공 (workdeck-dev + workdeck-prod)
- [x] 백필 후 Space 수 = 기존 Workspace 수 (dev: 0, prod: 1=1 일치 확인)
- [x] 기존 coupang-ads 기능 회귀 없음 (`npm run build` 통과)
- [x] Vercel 프로덕션 배포 READY (`https://app.workdeck.work` 정상)
