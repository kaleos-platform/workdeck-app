// 네이버 블로그 브라우저 자동화 퍼블리셔 (bo 버전) — Playwright storageState 기반.
...
//     따라서 modal open 후에는 ESC 로직을 실행하지 않는다.

import { chromium, type Browser, type Page, type Frame } from 'playwright'
import type { BoPublisher, BoPublishResult } from './index.js'
import type { BoPublishContext } from '../contracts.js'
import { renderDocToPlainText } from './_naver-doc-text.js'

export class NaverBlogBrowserPublisher implements BoPublisher {
  readonly name = 'bo-naver-blog-browser'
  ...
  async publish(ctx: BoPublishContext): Promise<BoPublishResult> {
    ...
    try {
      ...
      await page.keyboard.type(ctx.variant.title, { delay: 20 })
