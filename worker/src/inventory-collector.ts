...
const VENDOR_METRICS_URL = `${WING_URL}/tenants/vendor-item-insights`
...
async function downloadInventoryHealth(
  ...
  const downloadBtn = page
    .locator(
      'button:has-text("엑셀"), button:has-text("다운로드"), a:has-text("엑셀"), a:has-text("다운로드")',
    )
    .first()
  ...
  const fileName = download.suggestedFilename() || `inventory_health_${Date.now()}.xlsx`
  const filePath = path.join(downloadDir, fileName)
  await download.saveAs(filePath)

  console.log(`[inventory]   → 재고 건강성 저장: ${fileName}`)
...
async function downloadVendorMetrics(
  ...
  const download = await downloadPromise
  const fileName = download.suggestedFilename() || `vendor_metrics_${Date.now()}.xlsx`
  const filePath = path.join(downloadDir, fileName)
  await download.saveAs(filePath)

  console.log(`[inventory]   → 판매 성과 저장: ${fileName}`)
  return { filePath, fileName }
}
