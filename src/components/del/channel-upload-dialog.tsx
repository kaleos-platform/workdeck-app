...
export function ChannelUploadDialog({
  ...
  async function handleFileUpload(selectedFile: File) {
    ...
    try {
      ...
      const emptySet = new Set(data.emptyColumns ?? [])
  ...
  function renderFieldRow(field: FieldDef) {
    ...
    return (
      <div key={field.value} className="flex items-start gap-3 px-3 py-2">
        <div className="w-24 shrink-0 pt-1.5 text-sm">
  ...
  return (
    ...
    >
      ...
      <DialogContent className="max-w-[1600px] w-[96vw] h-[92vh] max-h-[92vh] p-0 gap-0 flex flex-col overflow-hidden">
        ...
        <div className="flex-1 min-h-0 overflow-hidden">
          ...
          {step === 'mapping' && preview && (
            <div className="h-full grid grid-cols-[minmax(480px,2fr)_3fr]">
              {/* 좌: 매핑 */}
              <div className="min-w-0 overflow-y-auto border-r px-6 py-4 space-y-5">
              ...
              {/* 우: 샘플 데이터 (독립 스크롤, sticky header) */}
              <div className="min-w-0 flex flex-col bg-muted/20">
                <div className="px-6 py-3 border-b shrink-0 flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {preview.sampleRows.length}개 행 미리보기
                  </span>
                </div>
                        ...
                        {preview.headers.map((h, i) => (
                          <TableHead
                            ...
                            className={cn(
                              ...
                              hoveredColumnIdx === i && 'bg-primary/15',
                            )}
                        ...
                        <TableRow key={i}>
                          {row.map((cell, j) => (
                            <TableCell
                              ...
                              title={cell}
                              className={cn(
                                'text-xs max-w-[200px] truncate transition-colors',
                                hoveredColumnIdx === j && 'bg-primary/10',
                              )}
                              onMouseEnter={() => setHoveredColumnIdx(j)}
                              onMouseLeave={() => setHoveredColumnIdx(null)}
                            >
                              {cell}
                            </TableCell>
        ...
        <DialogFooter className="px-6 py-4 shrink-0 border-t sm:justify-between items-center gap-3">
          {step === 'upload' && <div className="flex-1" />}
          {step === 'mapping' && preview && (
            <>
              <span className="text-xs text-muted-foreground">
                매핑 완료 {mappedFields.length} / {FIELDS.length}
              </span>
