  ...
  async function handleSaveChannel() {
    ...
    try {
      ...
      const url = editingChannel
        ? `/api/del/channels/${editingChannel.id}`
        : '/api/del/channels'
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {channel.requireOrderNumber && (
                            <Badge variant="outline" className="text-xs">주문번호</Badge>
                          )}
                          {channel.requirePayment && (
                            <Badge variant="outline" className="text-xs">결제금액</Badge>
                          )}
                          {channel.requireProducts && (
                            <Badge variant="outline" className="text-xs">배송상품</Badge>
                          )}
                          {!channel.requireOrderNumber && !channel.requirePayment && !channel.requireProducts && (
    ...
    <>
      ...
      <Dialog open={groupDialogOpen} onOpenChange={setGroupDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setGroupDialogOpen(false)} disabled={savingGroup}>
              취소
            </Button>
