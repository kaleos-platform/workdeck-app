...
export function StockStatusTable() {
  ...
  useEffect(() => { setPage(1) }, [groupFilter, locationFilter])
  ...
  useEffect(() => {
    fetch('/api/inv/locations?isActive=true')
      .then(res => res.ok ? res.json() : null)
      .then(json => { if (json?.locations) setAllLocations(json.locations) })
      .catch(() => {})
  }, [])
            ...
            {loading && !data ? (
              ...
              data.data.map((row) => (
                <TableRow key={row.optionId}>
                  ...
                  <TableCell className={`text-right tabular-nums ${row.totalStock < 0 ? 'text-red-600' : ''}`}>
                  ...
                  {visibleLocations.map(loc => {
                    ...
                    return (
                      <TableCell key={loc.id} className={`text-right tabular-nums ${qty < 0 ? 'text-red-600' : qty === 0 ? 'text-muted-foreground' : ''}`}>
