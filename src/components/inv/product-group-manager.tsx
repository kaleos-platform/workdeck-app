...
export function ProductGroupManager({ open, onOpenChange, onChanged }: Props) {
  ...
  const handleDelete = async (group: GroupItem) => {
    if (!confirm(`"${group.name}" 그룹을 삭제하시겠습니까?\n소속 상품 ${group.productCount}개는 기본 그룹으로 이동됩니다.`)) return
    try {
                  ...
                  {editingId === g.id ? (
                    <>
                      <Input
                        ...
                        autoFocus
                        onKeyDown={(e) => { if (e.key === 'Enter') void handleRename(g.id); if (e.key === 'Escape') setEditingId(null) }}
                      />
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleRename(g.id)}>
                    ...
                    <>
                      ...
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditingId(g.id); setEditName(g.name) }}>
                        <Pencil className="h-3.5 w-3.5" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => handleDelete(g)}>
