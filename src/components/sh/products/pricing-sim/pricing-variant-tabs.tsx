...
export function PricingVariantTabs({
  ...
  return (
    <div className="mb-4 flex items-center gap-1 overflow-x-auto pb-1">
      {/* 표준 TabsList 룩 (bg-muted 트랙 + active는 bg-background/shadow-sm) — 인라인 편집·복제·삭제 때문에 커스텀 유지 */}
      <div className="inline-flex h-9 w-fit shrink-0 items-center gap-1 rounded-lg bg-muted p-[3px]">
        {tabs.map((tab) => {
          ...
          return (
            <div
              key={tab.id}
              className={cn(
                'group relative flex h-[calc(100%-1px)] shrink-0 items-center gap-0.5 rounded-md border border-transparent px-2.5 text-sm font-medium',
                active
