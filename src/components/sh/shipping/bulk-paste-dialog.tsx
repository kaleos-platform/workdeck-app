...
import { type OrderRow, createEmptyRow } from '@/components/sh/shipping/registration-table'
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="flex max-h-[80vh] max-w-5xl flex-col overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>
            엑셀에서 복사한 데이터를 아래에 붙여넣으세요. 탭으로 구분된 데이터를 자동으로
            파싱합니다.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto">
          <div className="space-y-2">
            <Label>컬럼 순서</Label>
            <p className="text-xs text-muted-foreground">{EXPECTED_COLUMNS.join(' → ')}</p>
          </div>
