...
export function ProductDetail({ productId, onClose }: { productId: string; onClose: () => void }) {
  ...
  const [optionDrafts, setOptionDrafts] = useState<Record<string, { name: string; sku: string }>>(
    {}
  )
    ...
    try {
      const res = await fetch(`/api/inv/products/${productId}/options`, {
        ...
        body: JSON.stringify({
          ...
          sku: newOption.sku.trim() || undefined,
        }),
      })
      if (!res.ok) {
        const err = await res.json().catch(() => ({}))
  ...
  if (loading) {
    return <div className="py-10 text-center text-muted-foreground">불러오는 중...</div>
  }
  ...
  return (
    <div className="space-y-6">
      <DialogHeader>
        ...
        <DialogDescription>상품명과 제품코드, 옵션 정보를 수정할 수 있습니다.</DialogDescription>
      </DialogHeader>
      ...
      <section className="space-y-3">
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          ...
          <div className="space-y-1">
            <label className="text-xs font-medium text-muted-foreground">제품코드</label>
            <Input
              value={codeDraft}
              placeholder="(없음)"
