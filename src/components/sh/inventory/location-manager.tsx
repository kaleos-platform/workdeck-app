    ...
    <div className="space-y-4">
          ...
          <TableBody>
            {loading ? (
              ...
              locations.map((loc) => {
                ...
                return (
                  <Fragment key={loc.id}>
                    <TableRow className="cursor-pointer" onClick={() => toggleExpand(loc.id)}>
                      <TableCell>
                        {expanded ? (
                      ...
                      <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                        <div className="inline-flex gap-1">
                          ...
                          {loc.isActive ? (
                            ...
                            <Button variant="ghost" size="sm" onClick={() => handleReactivate(loc)}>
                              활성화
                            </Button>
                          )}
                        </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{editing ? '위치 수정' : '새 보관 장소'}</DialogTitle>
            <DialogDescription>
