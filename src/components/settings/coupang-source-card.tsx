...
const SCOPE_ROWS: {
  ...
  },
  {
    ...
    label: '정산',
    // 매출내역 API 는 판매자배송(마켓플레이스)만 반환하고 로켓그로스는 구조적으로 빠진다.
    // 실측 2구간 교차검증: RG 정산이 실제로 존재하는 2026-07-06~12 구간에서도 API 옵션 25개와
    // RG 정산 엑셀 옵션 62개의 교집합이 0. Rocket Growth 계열 9개 엔드포인트에 정산이 없고
    // 매출내역에는 판매방식 필터 파라미터도 없어 우회 경로가 없다. 지급내역은 조회 결과 0건.
    // 주력이 로켓그로스라 절반만 자동화하면 Wing 수동 엑셀과 병행 운영이 되어 실익이 없다.
    description:
      '매출·지급 내역 데이터. API는 판매자배송만 제공하고 로켓그로스 정산이 빠져 있어 반쪽 데이터가 됩니다.',
    lockedBadge: '크롤링 (로켓그로스 미제공)',
  },
  {
    field: 'productSource',
    label: '상품',
