...
지켜야 할 것:
- 기준 상품명에 들어 있는 단어를 다시 쓰지 않습니다. 그 단어들을 붙여 만든 복합어도
  마찬가지입니다. 기준 상품명이 "쿨 메쉬 심리스 커버 브라 노와이어 중년 여성 속옷"이라면
  '노와이어브라'·'심리스브라'·'중년여성브라'·'여성속옷'은 모두 금지입니다 — 띄어 쓰든 붙여
  쓰든 상품명 단어만으로 이뤄진 검색어는 새 유입을 만들지 못합니다.
  반대로 상품명에 없는 단어가 하나라도 들어간 조합은 좋습니다:
  '갱년기여성속옷'(갱년기 ○), '티셔츠브라'(티셔츠 ○), '여름브라'(여름 ○).
...
[이미 등록된 검색어] 각각을 아래 라벨 하나로 판정합니다. 최대 ${REVIEW_LIMIT}개.
- KEEP              : 문제 없음. 유지.
- LOW_RELEVANCE     : 이 상품과 관련이 약해 유입이 어긋난다.
- COMPETITOR_BRAND  : 경쟁 브랜드명이다.
- FALSE_CLAIM       : 이 상품에 없는 기능·효능을 말한다.
- CATEGORY_STUFFING : 카테고리명을 나열했을 뿐이다.
- LOW_INTENT        : 구매 의도가 낮은 정보 탐색성 표현이다.
- MOVE_TO_OPTION    : 색상·사이즈·소재처럼 검색어가 아니라 "검색 옵션"으로 관리할 값이다.
문제가 없으면 KEEP 을 쓰고 억지로 지적하지 마세요. reason 은 40자 이내.
